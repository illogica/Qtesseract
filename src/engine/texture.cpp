    ...
    loopi(bh)
    {
        for(uchar *curdst = dst, *end = &src[bw*blocksize]; src < end; src += blocksize, curdst += stridex)
        {
            if(format == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT)
            {
                ...
                if(swapxy) tess_swap(xshift, yshift);
...
void texturereset(int *n)
{
    ...
    defslot = NULL;
    resetslotshader();
    int limit = clamp(*n, 0, tess_slots.length());
    for(int i = limit; i < tess_slots.length(); i++)
    {
        Slot *s = tess_slots[i];
        for(VSlot *vs = s->variants; vs; vs = vs->next) vs->slot = &dummyslot;
        delete s;
    }
    tess_slots.setsize(limit);
    while(vslots.length())
    {
...
int compactvslots(bool cull)
{
    ...
    if(cull)
    {
        int numdefaults = min(int(NUMDEFAULTSLOTS), tess_slots.length());
        loopi(numdefaults) tess_slots[i]->variants->index = compactedvslots++;
        loopi(numdefaults) assignvslotlayer(*tess_slots[i]->variants);
    }
    else
    {
        loopv(tess_slots) tess_slots[i]->variants->index = compactedvslots++;
        loopv(tess_slots) assignvslotlayer(*tess_slots[i]->variants);
        loopv(vslots)
        {
            VSlot &vs = *vslots[i];
    ...
    if(cull)
    {
        loopvrev(tess_slots) if(tess_slots[i]->variants->index < 0) delete tess_slots.remove(i);
        loopv(tess_slots) tess_slots[i]->index = i;
    }
    loopv(vslots)
    {
        while(vslots[i]->index >= 0 && vslots[i]->index != i)
            tess_swap(vslots[i], vslots[vslots[i]->index]);
    }
...
static void clampvslotoffset(VSlot &dst, Slot *slot = NULL)
{
    ...
    if(slot && slot->sts.inrange(0))
    {
        ...
        if((dst.rotation&5)==1) tess_swap(xs, ys);
...
{
    int offset = 0;
    loopvrev(tess_slots) if(tess_slots[i]->variants) { offset = tess_slots[i]->variants->index + 1; break; }
...
void texture(char *type, char *name, int *rot, int *xoffset, int *yoffset, float *scale)
{
    ...
    if(tnum==TEX_DIFFUSE)
    {
        if(tess_slots.length() >= 0x10000) return;
        defslot = tess_slots.add(new Slot(tess_slots.length()));
    }
...
void texlayer(int *layer)
{
    if(!defslot) return;
    Slot &s = *defslot;
    s.variants->layer = *layer < 0 ? max(tess_slots.length()-1+*layer, 0) : *layer;
...
void texdetail(int *detail)
{
    if(!defslot) return;
    Slot &s = *defslot;
    s.variants->detail = *detail < 0 ? max(tess_slots.length()-1+*detail, 0) : *detail;
...
{
    Slot &s = tess_slots.inrange(index) ? *tess_slots[index] : (tess_slots.inrange(DEFAULT_GEOM) ? *tess_slots[DEFAULT_GEOM] : dummyslot);
...
{
    VSlot &s = vslots.inrange(index) && vslots[index]->slot ? *vslots[index] : (tess_slots.inrange(DEFAULT_GEOM) && tess_slots[DEFAULT_GEOM]->variants ? *tess_slots[DEFAULT_GEOM]->variants : dummyvslot);
    if(load && !s.linked)
    {
...
{
    loopv(tess_slots) if(tess_slots[i]->loaded) linkslotshader(*tess_slots[i]);
    ...
    loopi(6)
    {
        ...
        if(blur > 0)
        {
            ...
            loopj(2)
            {
                ...
                tess_swap(emfbo[0], emfbo[1]);
                tess_swap(emtex[0], emtex[1]);
            }
        }
...
{
    clearenvmaps();
    loopv(tess_slots) tess_slots[i]->cleanup();
