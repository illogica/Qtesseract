...
struct pvsworker
{
    ...
    void cullpvs(pvsnode &p, const ivec &co = ivec(0, 0, 0), int size = worldsize)
    {
        ...
        if(p.children && !(p.flags&PVS_HIDE_BB))
        {
            ...
            if(order[2].dist < order[1].dist) tess_swap(order[1], order[2]);
            if(order[3].dist < order[2].dist) tess_swap(order[2], order[3]);
            if(order[2].dist < order[1].dist) tess_swap(order[1], order[2]);
        ...
        if(order[1].dist > order[0].dist) tess_swap(order[0], order[1]);
        if(order[2].dist > order[1].dist) tess_swap(order[1], order[2]);
        if(order[1].dist > order[0].dist) tess_swap(order[0], order[1]);
        loopi(6)
        {
