...
struct soundconfig
{
    int tess_slots, numslots;
    int maxuses;

    bool hasslot(const soundslot *p, const vector<soundslot> &v) const
    {
        return p >= v.getbuf() + tess_slots && p < v.getbuf() + tess_slots+numslots && tess_slots+numslots < v.length();
    }

    int chooseslot() const
    {
        return numslots > 1 ? tess_slots + rnd(numslots) : tess_slots;
    }
};
...
static struct soundtype
{
    ...
    int findsound(const char *name, int vol)
    {
        loopv(configs)
        {
            ...
            loopj(s.numslots)
            {
                soundslot &c = tess_slots[s.tess_slots+j];
    ...
    int addslot(const char *name, int vol)
    {
        ...
        soundslot *oldslots = tess_slots.getbuf();
        int oldlen = tess_slots.length();
        soundslot &slot = tess_slots.add();
        // soundslots.add() may relocate slot pointers
        if(tess_slots.getbuf() != oldslots) loopv(channels)
        {
            soundchannel &chan = channels[i];
            if(chan.inuse && chan.slot >= oldslots && chan.slot < &oldslots[oldlen])
                chan.slot = &tess_slots[chan.slot - oldslots];
        }
        slot.sample = s;
    ...
    void reset()
    {
        ...
        {
            soundchannel &chan = channels[i];
            if(chan.inuse && tess_slots.inbuf(chan.slot))
            {
                Mix_HaltChannel(i);
                freechannel(i);
            }
    ...
    void preloadsound(int n)
    {
        ...
        loopk(config.numslots) tess_slots[config.tess_slots+k].sample->load(dir, true);
    }

    bool playing(const soundchannel &chan, const soundconfig &config) const
    {
        return chan.inuse && config.hasslot(chan.slot, tess_slots);
    }
...
int playsound(int n, const vec *loc, extentity *ent, int flags, int loops, int fade, int chanid, int radius, int expire)
{
    ...
    soundslot &slot = sounds.tess_slots[config.chooseslot()];
