...
void copyhdr(int sw, int sh, GLuint fbo, int dw, int dh, bool flipx, bool flipy, bool swapxy)
{
    ...
    if(swapxy) tess_swap(reorientx, reorienty);
    reorientx.mul(sw);
    reorienty.mul(sh);
...
void processhdr(GLuint outfbo, int aa)
{
    ...
    if(hdrreduce) while(pw > bloomw || ph > bloomh)
    {
        ...
        pw = cw;
        ph = ch;
        tess_swap(b0fbo, b1fbo);
        tess_swap(b0tex, b1tex);
        tess_swap(b0w, b1w);
        tess_swap(b0h, b1h);
    }
...
void rendervolumetric()
{
    ...
    if(volbilateral || volblur)
    {
        tess_swap(volfbo[0], volfbo[1]);
        tess_swap(voltex[0], voltex[1]);
    }
...
static void batchlights(const batchstack &initstack)
{
    ...
    while(numstack > 0)
    {
        ...
        for(int i = outside; i < inside; ++i)
        {
            ...
            if(r.outside(s))
            {
                if(i != outside) tess_swap(batchrects[i], batchrects[outside]);
                ++outside;
            }
            else if(s.inside(r))
            {
                ++groups[r.group];
                tess_swap(batchrects[i--], batchrects[--inside]);
            }
        ...
        loop(g, BF_NOSUN) while(groups[g] >= lighttilebatch || (inside == outside && (groups[g] || !(flags & BF_NOSUN))))
        {
            ...
            loopi(n)
            {
                ...
                tess_swap(batchrects[best], batchrects[--batched]);
            }
...
void radiancehints::renderslices()
{
    ...
    if(!rhrect)
    {
        ...
        if(rhcache)
        {
            loopi(4) tess_swap(rhtex[i], rhtex[i+4]);
