...
ICOMMAND(selswap, "", (), { if(noedit(true)) return; tess_swap(sel, savedsel); });
...
{
    if(*idx < 0 || !tess_slots.inrange(*idx)) { intret(-1); return; }
...
ICOMMAND(numslots, "", (), intret(tess_slots.length()));
...
{
    tess_swap(c.texture[d*2], c.texture[d*2+1]);
    c.faces[D[d]] = dflip(c.faces[D[d]]);
    c.faces[C[d]] = cflip(c.faces[C[d]]);
    c.faces[R[d]] = rflip(c.faces[R[d]]);
    if(c.children)
    {
        loopi(8) if(i&octadim(d)) tess_swap(c.children[i], c.children[i-octadim(d)]);
...
void rotatecube(cube &c, int d)   // rotates cube clockwise. see pics in cvs for help.
{
    ...
    tess_swap(c.faces[R[d]], c.faces[C[d]]);

    tess_swap(c.texture[2*R[d]], c.texture[2*C[d]+1]);
    tess_swap(c.texture[2*C[d]], c.texture[2*R[d]+1]);
    tess_swap(c.texture[2*C[d]], c.texture[2*C[d]+1]);
    ...
    if((texpaneltimer -= curtime)>0 && editmode)
    {
        ...
        loopi(7)
        {
            ...
            if(texmru.inrange(ti))
            {
                ...
                if(vslot.rotation)
                {
                    if((vslot.rotation&5) == 1) { tess_swap(xoff, yoff); loopk(4) tess_swap(tc[k].x, tc[k].y); }
